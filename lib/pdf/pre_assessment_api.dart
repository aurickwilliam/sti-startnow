...
class PreAssessmentApi {
  ...
  static Future<File> generatePreAssessment(Student student) async {
    ...
    final stiLogo =
        (await rootBundle.load(
          "assets/img/sti_caloocan_logo.png",
        )).buffer.asUint8List();
    ...
    double totalUnits() {
      ...
      for (var innerList in student.enrollment.subjectList) {
    ...
    return SaveAndOpenPdf.savePdf(
      name: "pre-assessment-${student.lastName?.toLowerCase()}.pdf",
      pdf: pdf,
    );
  }
  ...
  static Widget header(Uint8List stiLogo) {
    ...
    return Row(
      children: [
        Container(height: 75, width: 75, child: Image(MemoryImage(stiLogo))),
        ...
        Container(
          child: Align(
            ...
            child: Text(getDateTime(), style: TextStyle(fontSize: 10)),
          ),
        ),
      ],
    );
        ...
        children: [
          Row(
            children: [
              ...
              tableCell(
                value: student.enrollment.academicStatus ?? ' No Status',
              ),
            ],
          ),
          Row(
